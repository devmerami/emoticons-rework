...
{
    private static final Field MODEL_BOX_QUADS = ReflectionHelper.findField(ModelBox.class, "quadList", "field_78254_i");
    ...
    public static ClientModel bakeBipedCustomArmorModel(ModelBiped model, ItemArmor armorItem)
    {
        ...
        List<ModelPartBind> allBoxes = Lists.newArrayList();
        ...
        return new ClientModel(bakeMeshFromCubes(allBoxes));
    }
    ...
    static TexturedQuad[] getQuads(ModelBox cube)
    {
        try
        {
            return (TexturedQuad[]) MODEL_BOX_QUADS.get(cube);
        }
        catch (IllegalAccessException e)
        {
            throw new RuntimeException("Failed to access model box quads", e);
        }
    }
    ...
    static Vec3f getNormalFrom(TexturedQuad quad, VisibleMatrix4f matrix)
    {
        ...
        Vec4f norm = new Vec4f((float) vec3d2.x, (float) vec3d2.y, (float) vec3d2.z, 0.0F);
    ...
    static class TexturedJointQuad
    {
        ...
        {
            this.animatedVertexPositions = positionsIn;
            positionsIn[0] = new PositionTextureJointVertex(positionsIn[0].setTexturePosition(positionsIn[0].texturePositionX, positionsIn[0].texturePositionY + cor), positionsIn[0].jointId, positionsIn[0].weight);
            positionsIn[1] = new PositionTextureJointVertex(positionsIn[1].setTexturePosition(positionsIn[1].texturePositionX, positionsIn[1].texturePositionY + cor), positionsIn[1].jointId, positionsIn[1].weight);
            positionsIn[2] = new PositionTextureJointVertex(positionsIn[2].setTexturePosition(positionsIn[2].texturePositionX, positionsIn[2].texturePositionY - cor), positionsIn[2].jointId, positionsIn[2].weight);
            positionsIn[3] = new PositionTextureJointVertex(positionsIn[3].setTexturePosition(positionsIn[3].texturePositionX, positionsIn[3].texturePositionY - cor), positionsIn[3].jointId, positionsIn[3].weight);
            this.normal = normal;
        }
    }
    ...
    static class Limb extends ModelPart
    {
        ...
        {
            List<TexturedJointQuad> quads = Lists.newArrayList();

            for (TexturedQuad quad : getQuads(cube))
            {
    ...
    static class Chest extends ModelPart
    {
        ...
        final WeightPair[] cutYList = {
            new WeightPair(13.6666F, 0.254F, 0.746F),
            new WeightPair(15.8333F, 0.254F, 0.746F),
            new WeightPair(18.0F, 0.5F, 0.5F),
            new WeightPair(20.1666F, 0.744F, 0.256F),
            new WeightPair(22.3333F, 0.770F, 0.230F)
        };

        @Override
        public void bakeCube(VisibleMatrix4f matrix, ModelBox cube, List<VertexData> vertices, List<Integer> indices)
        {
            List<TexturedJointQuad> seperatedX = Lists.newArrayList();
            List<TexturedJointQuad> seperatedXY = Lists.newArrayList();
            ...
            for (TexturedJointQuad quad : seperatedX)
            {
                ...
                List<PositionTextureJointVertex> addedVertexList = Lists.newArrayList();
                addedVertexList.add(pos0);
                addedVertexList.add(pos1);

                if (!weightPairList.isEmpty())
                {
        ...
        {
            List<WeightPair> cutYs = Lists.newArrayList();

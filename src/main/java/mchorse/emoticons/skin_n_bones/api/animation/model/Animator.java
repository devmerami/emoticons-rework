...
public class Animator implements IAnimator
{
    ...
    protected void controlActions(EntityLivingBase target)
    {
        ...
        else if (target.isOnLadder())
        {
            boolean climbing = Math.abs(target.motionY) > 0.01D || target.limbSwingAmount > 0.01F;

            this.setActiveAction(climbing ? this.walking : this.idle);

            if (this.walking != null)
            {
                float climbSpeed = target.limbSwingAmount > 0.01F ? target.limbSwingAmount * 1.16F : (float) Math.abs(target.motionY) * 6.0F;
                this.walking.setSpeed(target.motionY < -0.01D ? -climbSpeed : climbSpeed);
            }
        }
        else if (wet)
        {
        ...
        if (target.isSwingInProgress && target.swingProgress == 0 && !target.isPlayerSleeping() && target.getHeldItemMainhand().isEmpty())
        {

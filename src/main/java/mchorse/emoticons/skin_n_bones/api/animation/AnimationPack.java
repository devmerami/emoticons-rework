...
public class AnimationPack implements IResourcePack
{
    ...
    public InputStream getInputStream(ResourceLocation location) throws IOException
    {
        ...
        return new FileInputStream(new File(this.config, ResourceLocationUtils.getPath(location)));
    }

    @Override
    public boolean resourceExists(ResourceLocation location)
    {
        if (new File(this.config, ResourceLocationUtils.getPath(location)).exists())
        {
            return true;
        }

        this.stream = AnimationPack.class.getResourceAsStream("/assets/skin_n_bones/models/" + ResourceLocationUtils.getPath(location));

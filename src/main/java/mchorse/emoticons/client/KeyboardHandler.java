...
import mchorse.emoticons.client.gui.GuiEmoteWheel;
...
public class KeyboardHandler
{
    ...
    public KeyBinding rollForward;
    public KeyBinding rollBackward;
    ...
    public KeyboardHandler()
    {
        ...
        this.rollForward = new KeyBinding(pre + "roll_forward", Keyboard.KEY_Z, pre + "category");
        this.rollBackward = new KeyBinding(pre + "roll_backward", Keyboard.KEY_X, pre + "category");
        ...
        ClientRegistry.registerKeyBinding(this.rollForward);
        ClientRegistry.registerKeyBinding(this.rollBackward);
    }
    ...
    public void onInputUpdate(InputUpdateEvent event)
    {
        ...
        Cosmetic cosmetic = (Cosmetic) cap;

        if (!cosmetic.isMovementInputLocked())
        {
            return;
        }
        ...
        if (cosmetic.isAttackMovementLocked())
        {
            player.motionX = 0.0D;
            player.motionZ = 0.0D;
        }
    ...
    private void processKeybind(EntityPlayer player, ICosmetic cap)
    {
        ...
        Cosmetic cosmetic = cap instanceof Cosmetic ? (Cosmetic) cap : null;
        ...
        if (this.emotes.isPressed())
        {
            ...
            if (minecraft.currentScreen instanceof GuiEmoteWheel)
            {
                minecraft.displayGuiScreen(null);
            }
            else if (minecraft.currentScreen == null)
            {
                minecraft.displayGuiScreen(new GuiEmoteWheel());
            }
        }
        ...
        if (cosmetic != null && player instanceof AbstractClientPlayer)
        {
            if (this.rollForward.isPressed())
            {
                this.tryRoll(cosmetic, (AbstractClientPlayer) player, true);
            }

            if (this.rollBackward.isPressed())
            {
                this.tryRoll(cosmetic, (AbstractClientPlayer) player, false);
            }
        }
    }

    private void tryRoll(Cosmetic cosmetic, AbstractClientPlayer player, boolean forward)
    {
        if (cosmetic.playRoll(player, forward))
        {
            Dispatcher.sendToServer(new PacketEpicFightAction(0, forward ? PacketEpicFightAction.ROLL_FORWARD : PacketEpicFightAction.ROLL_BACKWARD));
        }
    }
}

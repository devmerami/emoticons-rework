...
public class EntityModelHandler
{
    ...
    public void onRenderPlayer(RenderPlayerEvent.Pre event)
    {
        ...
        {
            ((Cosmetic) cap).setRenderer(event.getRenderer());
        }
    ...
    public void onHUDRender(RenderGameOverlayEvent.Post event)
    {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.ALL)
        {
            ...
            if (cap != null && this.canShow(cap) && mc.gameSettings.thirdPersonView == 0 && !this.shouldHideFirstPersonPreview(player, cap))
            {
    ...
    private boolean shouldHideFirstPersonPreview(EntityPlayer player, ICosmetic cap)
    {
        if (!(cap instanceof Cosmetic))
        {
            return cap.getEmote() != null;
        ...
        return ((Cosmetic) cap).shouldHideFirstPersonPreview(player);
    }
